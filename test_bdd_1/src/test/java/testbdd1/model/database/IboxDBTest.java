package testbdd1.model.database;
...
public class IboxDBTest {
	...
	public void test(int max) {
		...
		iBoxDB.LocalServer.DB.root("src/main/resources/bdd");
		DB db = new DB(1, "");
		// DB server = new DB("src/main/resources/bdd");
		db.getConfig().ensureIndex(COLLECTION_NAME, Personne.class, "ID");
		AutoBox box = db.open();
		// server.getConfig().ensureTable(COLLECTION_NAME, Personne.class,
		// "ID");
		// server.getConfig().ensureTable(COLLECTION_NAME, Personne.class,
		...
		box.insert(COLLECTION_NAME, p);
		Personne o1 = box.selectKey(Personne.class, COLLECTION_NAME, "p0");
		System.out.println(o1.toString());
		// o1.Name = "Kelly";
		// db.update("Table", o1);
		box.delete(COLLECTION_NAME, "p0");
		db.close();

		// int i;
		// long deb = System.currentTimeMillis();
		// for (i = 0; i < max; i++) {
		// Personne p = createPersonne("p" + i);
		// // map.put("p" + i, p);
		// // db.commit();
		// }
